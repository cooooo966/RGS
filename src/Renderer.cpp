...
namespace RGS {
	...
	bool Renderer::IsInsideTriangle(float(&weight)[3]) {
		return weight[0] >= -EPSILON && weight[1] >= -EPSILON && weight[2] >= -EPSILON;
	}
	...
	bool Renderer::PassDepthTest(const float writeDepth, const float fDepth, const DepthFuncType depthType) {
		switch (depthType)
		{
	...
	void Renderer::CalculateWeights(float(&screenWeights)[3], float(&weights)[3], const Vec4(&fragCoords)[3], const Vec2& screenPoint) {
		...
		float s = (ap.X * ac.Y - ap.Y * ac.X) * factor;
		float t = (ab.X * ap.Y - ap.X * ab.Y) * factor;

		screenWeights[0] = 1 - s - t;
		screenWeights[1] = s;
		screenWeights[2] = t;

		float w0 = screenWeights[0] * fragCoords[0].W;
		float w1 = screenWeights[1] * fragCoords[1].W;
		float w2 = screenWeights[2] * fragCoords[2].W;
